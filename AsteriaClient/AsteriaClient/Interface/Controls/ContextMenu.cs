...
namespace AsteriaClient.Interface.Controls
{
    public class ContextMenu : MenuBase
    {
        ...
        protected override void DrawControl(Renderer renderer, Rectangle rect, GameTime gameTime)
        {
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                if (ItemIndex != i)
                {
                    ...
                    {
                        Rectangle r = new Rectangle(left + l1.Text.OffsetX, top + l1.Text.OffsetY, LineWidth() - vsize, h);
                ...
                else
                {
                    if (Items[i].Enabled)
                    {
                        Rectangle rs = new Rectangle(rect.Left + l1.ContentMargins.Left, top, Width - (l1.ContentMargins.Horizontal - Skin.OriginMargins.Horizontal), h);
                    ...
                    {
                        Rectangle rs = new Rectangle(rect.Left + l1.ContentMargins.Left, top, Width - (l1.ContentMargins.Horizontal - Skin.OriginMargins.Horizontal), vsize);
        ...
        private bool CheckArea(int x, int y)
        {
            if (Visible)
            {
                if (x <= AbsoluteLeft || x >= AbsoluteLeft + Width || y <= AbsoluteTop || y >= AbsoluteTop + Height)
                {
                    bool ret = false;
                    if (ChildMenu != null)
